...
class UI(tk.Frame):
    ...
    def paste_from_clipboard(self, target: tk.Text):
        """Colle le contenu du presse-papiers dans la zone passée (remplace la sélection si présente)."""
        try:
        ...
        try:
            # Si une sélection existe, on remplace
            start = target.index("sel.first")
            end = target.index("sel.last")
            target.delete(start, end)
            target.insert(start, data)
        except tk.TclError:
            # Sinon on insère à la fin (avec saut de ligne si nécessaire)
            content = target.get("1.0", tk.END).strip()
            if content and not content.endswith("\n"):
                target.insert(tk.END, "\n")
            target.insert(tk.END, data)
    ...
    def create_widgets(self):
        ...
        # Définir une voix française par défaut
        ...
        src_paste_btn = tk.Button(src_btns, text="📋", width=8,
                                command=lambda: self.paste_from_clipboard(self.src_txt))
        src_paste_btn.pack(side="left", padx=(0, 4))

        src_clear_btn = tk.Button(src_btns, text="✖", width=8,
                                command=lambda: self.src_txt.delete("1.0", tk.END))
        ...
        self.preview_src_btn = tk.Button(src_btn_frame, text=self.get_text("preview_source"),
                                        command=self.preview_source, font=("Segoe UI", 9))
        self.preview_src_btn.pack(side="left", fill="x", expand=True, padx=(0, 2))
        
        self.stop_src_btn = tk.Button(src_btn_frame, text="⏹", command=self.stop_audio,
                                    state="disabled", font=("Segoe UI", 9), width=3)
        ...
        # Définir une voix anglaise par défaut
        ...
        tgt_paste_btn = tk.Button(tgt_btns, text="📋", width=8,
                                command=lambda: self.paste_from_clipboard(self.tgt_txt))
        tgt_paste_btn.pack(side="left", padx=(0, 4))

        tgt_clear_btn = tk.Button(tgt_btns, text="✖", width=8,
                                command=lambda: self.tgt_txt.delete("1.0", tk.END))
        tgt_clear_btn.pack(side="left")
        ...
        self.preview_tgt_btn = tk.Button(tgt_btn_frame, text=self.get_text("preview_translation"),
                                        command=self.preview_translation, font=("Segoe UI", 9))
        self.preview_tgt_btn.pack(side="left", fill="x", expand=True, padx=(0, 2))
        
        self.stop_tgt_btn = tk.Button(tgt_btn_frame, text="⏹", command=self.stop_audio,
                                    state="disabled", font=("Segoe UI", 9), width=3)
        ...
        self.temp_cards_label = tk.Label(self.right_frame, text=self.get_text("temp_cards"),
                                        bg="#f9f1da", font=("Segoe UI", 10, "bold"))
    ...
    def _add_textarea_with_tools(self, parent, height=3, pady=(0, 5)):
        """
        Construit UNE LIGNE complète contenant :
        - un Text (expand)
        - une colonne de deux boutons : 📋 (coller), ✖ (vider)
        Retourne le widget Text créé.
        """
        row = tk.Frame(parent, bg="#f9f1da")
        row.pack(fill="x", pady=pady)

        txt = tk.Text(row, height=height)
        txt.pack(side="left", fill="x", expand=True)

        tools = tk.Frame(row, bg="#f9f1da")
        tools.pack(side="left", padx=(6, 0), fill="y")

        # Bouton coller
        paste_btn = tk.Button(tools, text="📋", width=3,
                            command=lambda: self._paste_from_clipboard(txt))
        paste_btn.pack(pady=(0, 3))

        # Bouton vider
        clear_btn = tk.Button(tools, text="✖", width=3,
                            command=lambda: txt.delete("1.0", tk.END))
        clear_btn.pack()

        return txt

    def _paste_from_clipboard(self, target: tk.Text):
        """Colle le presse-papiers dans 'target' : remplace la sélection si présente, sinon insère à la fin."""
        try:
            data = self.master.clipboard_get()
        except tk.TclError:
            data = ""
        if not data:
            try: self.master.bell()
    ...
    def _show_loading_dialog(self, msg="Chargement…"):
        ...
        if not hasattr(self, "loading_win"):
            self.loading_win = None
        if not hasattr(self, "_loading_pb"):
            self._loading_pb = None

        # Si une fenêtre précédente traîne, on la ferme
        try:
        ...
        win.title(self.get_text("loading"))
        win.configure(bg="#f9f1da")
        win.resizable(False, False)
        win.transient(self.master)
        win.grab_set()

        lbl = tk.Label(win, text=msg, bg="#f9f1da")
        lbl.pack(padx=20, pady=(14, 8))

        pb = ttk.Progressbar(win, mode="indeterminate", length=240)
        pb.pack(padx=20, pady=(0, 14))
        pb.start(12)

        # Forcer calcul des dimensions pour pouvoir centrer
    ...
    def generate(self):
        ...
        self._show_loading_dialog("Generating deck…")
        ...
        def _worker():
            try:
                async def _run():
                    ...
                    # Déplacer les mp3 dans le dossier
                ...
                # Exécuter l'async dans un event loop propre au thread
                loop = asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
                try:
                    apkg_path = loop.run_until_complete(_run())
                finally:
                    loop.close()

                # UI thread : fermer loader + succès
                self.master.after(0, lambda: (
                    self._hide_loading_dialog(),
                    messagebox.showinfo(self.get_text("done"), f"Dossier généré → {apkg_path}")
                ))

            except Exception as e:
                # UI thread : fermer loader + erreur
                self.master.after(0, lambda: (
                    self._hide_loading_dialog(),
